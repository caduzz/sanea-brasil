  ...
  return (
    <header className="w-full h-14 bg-[#235D89] px-4 py-2 relative">
      <nav className="w-full h-full flex items-center justify-between md:justify-center">
        ...
        >
          <span className={`block w-6 h-0.5 bg-white mb-1 transition-all ${menuOpen ? "rotate-45 translate-y-1" : ""}`}></span>
        ...
        >
          ...
          <Dropdown.Root title={texts[language].submissions}>
            <Dropdown.Button>{texts[language].modelApresentation}</Dropdown.Button>
            <Dropdown.Button>{texts[language].modelEposter}</Dropdown.Button>
          </Dropdown.Root>
          <Dropdown.Root title={texts[language].exposanea}>
            <Dropdown.Button>{texts[language].exhibitors}</Dropdown.Button>
          </Dropdown.Root>
          <NavButton>{texts[language].register}</NavButton>
        </div>
